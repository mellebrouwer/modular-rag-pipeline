...
class BaseRelationalDBRetriever(PipelineComponent):
    def validate_input_data(self, data: Data):
        if not hasattr(data, "routing_table") or not isinstance(data.routing_table, RoutingTable):
            raise ValueError("RelationalDBRetriever requires a valid RoutingTable in the data.")
        queries = self.get_queries(data)
        if not isinstance(queries, list):
            raise ValueError(
                f"RelationalDBRetriever expected a list[Query], got {type(queries).__name__}"
            )
        if not queries:
            raise ValueError(
                "RelationalDBRetriever expected a non-empty list[Query], got an empty list"
            )
        if not isinstance(queries[0], Query):
            raise ValueError(
                f"RelationalDBRetriever expected a list[Query], got {type(queries[0]).__name__}"
            )
    ...
    def get_queries(data: Data) -> list[Query]:
        return data.routing_table.get_queries_for(Destination.RELATIONAL_DB)
